function find_addr() {
	...
	$(document).ready(function(){
		...
		for(var i = 0 ; i < sel_hp.options.length; i ++){
			if(sel_hp.options[i].value == "${nonmemberInfo.nonmember_hp1}"){
		...
		for(var i = 0 ; i < sel_cp.options.length; i ++){
			if(sel_cp.options[i].value == "${nonmemberInfo.nonmember_cp1}"){
	...
	function Sub(){
		...
		if(num == 0){
		document.updateform.action="nonmemberupdate.do";
